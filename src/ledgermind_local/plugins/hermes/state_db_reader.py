...
def _digest_with_round_override(round_digest: str | None) -> str | None:
    if not round_digest:
        return None
    value = str(round_digest)
    return value if value.startswith("sha256:") else f"sha256:{value}"


def _canonical_message_for_digest(row: sqlite3.Row, columns: set[str]) -> dict[str, Any]:
    ...
    for name in (
        ...
        if name not in columns:
            continue
        value = row[name]
        payload[name] = _safe_json_parse(value) if name == "tool_calls" else value
    return payload


def _compute_full_round_digest(
    *,
    session_id: str | None,
    source_round_id: str,
    messages: list[sqlite3.Row],
    ...
    payload = {
        "session_id": session_id or "",
        "source_round_id": source_round_id,
        "messages": [_canonical_message_for_digest(row, columns) for row in messages],
    }
    raw = json.dumps(payload, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
    return f"sha256:{hashlib.sha256(raw.encode('utf-8')).hexdigest()}"
...
def _resolve_from_messages(
    ...
    *,
    ...
    return ResolvedRoundReference(
        ...
        message_ids=ids,
        source_digest=(
            _digest_with_round_override(round_digest)
            or _compute_full_round_digest(
                session_id=session_id,
                source_round_id=round_id,
                messages=selected,
                columns=columns,
...
def _resolve_explicit_from_state(
    *,
    connection: sqlite3.Connection,
    columns: set[str],
    session_id: str,
    ...
    requested = list(explicit_message_ids)
    if not requested and first_message_id:
        requested.append(first_message_id)
    if not requested and final_message_id:
        requested.append(final_message_id)
    if first_message_id and first_message_id not in requested:
        requested.insert(0, first_message_id)
    if final_message_id and final_message_id not in requested:
        requested.append(final_message_id)
    if not requested:
        return None

    selected_columns = [
        _MESSAGE_COLUMN_ALIASES[name]
        for name in (
            "id",
            "session_id",
            "role",
    ...
    try:
        numeric_first = int(first_message_id) if first_message_id else None
        numeric_final = int(final_message_id) if final_message_id else None
    except (TypeError, ValueError):
        numeric_first = numeric_final = None

    if numeric_first is not None and numeric_final is not None:
        if numeric_first > numeric_final:
            return None
        rows = list(
            connection.execute(
                f"SELECT {selected_sql} FROM messages "
                "WHERE session_id = ? AND id BETWEEN ? AND ? ORDER BY id ASC",
                (session_id, numeric_first, numeric_final),
            ).fetchall()
        )
    else:
        ...
        if any(message_id not in by_id for message_id in requested):
            return None
        rows = [by_id[message_id] for message_id in requested]

    if not rows or str(rows[0]["id"]) != (first_message_id or str(rows[0]["id"])):
        return None
    if final_message_id and str(rows[-1]["id"]) != final_message_id:
        return None
...
def _resolve_by_explicit_ids(
    *,
    ...
    if not ids and first_message_id:
        ids.append(first_message_id)
    if not ids and final_message_id:
        ids.append(final_message_id)
    if first_message_id and first_message_id not in ids:
        ids.insert(0, first_message_id)
    if final_message_id and final_message_id not in ids:
        ids.append(final_message_id)
    ...
    return ResolvedRoundReference(
        source_session_id=session_id,
        source_round_id=round_id,
        first_message_id=first_message_id or (ids_tuple[0] if ids_tuple else None),
        final_message_id=final_message_id or (ids_tuple[-1] if ids_tuple else None),
        message_ids=ids_tuple,
        source_digest=(
            _digest_with_round_override(round_digest)
            or _compute_digest(
...
def resolve_round_reference(
    *,
    ...
    if explicit_first or explicit_final or explicit_ids:
        if state_db_path is not None and _to_str(session_id):
            try:
                ...
                try:
                    candidate = _resolve_explicit_from_state(
                        connection=connection,
                        columns=columns,
    ...
    return ResolvedRoundReference(
        ...
        source_schema_version=source_schema_version,
        resolver_version=resolver_version,
        verified=bool(preferred_round_id),
    )
