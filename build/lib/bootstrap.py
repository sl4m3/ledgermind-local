...
DEFAULT_PROJECTIONS: tuple[str, ...] = ("projections.search", "projections.knowledge")
...
class _CoreOutboxEventRepository(EventRepository):
    ...
    def __init__(self, delegate: SQLiteOutboxRepository) -> None:
        self._delegate = delegate

    def add(self, event: DomainEvent) -> None:
        row = _to_sqlite_outbox_event(event)
        self._delegate.add(row, projection_names=DEFAULT_PROJECTIONS)
...
class _SQLiteCoreUnitOfWork(UnitOfWork):
    ...
    def __enter__(self) -> "_SQLiteCoreUnitOfWork":
        ...
        self._events = _CoreOutboxEventRepository(uow.outbox)
        self._search = SQLiteKnowledgeSearchAdapter(uow.connection)
        return self
