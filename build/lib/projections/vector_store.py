...
class VectorProjectionStore:
    ...
    def _load_if_exists(self) -> None:
        ...
        if not isinstance(ids, list):
            raise ValueError("ids.json is malformed")
    ...
    def _coerce_id(self, value: object) -> str:
        if not isinstance(value, str):
            raise ValueError("knowledge_id must be a string")
        return value
    ...
    def _verify_candidate(self, candidate: Path) -> None:
        ...
        if not isinstance(ids, list):
            raise ValueError("candidate ids is malformed")
        if not isinstance(manifest, dict):
            raise ValueError("candidate manifest is malformed")
        if not isinstance(vectors, list):
            raise ValueError("candidate vectors is malformed")
    ...
    def _load_json(self, path: Path) -> dict[str, Any] | list[Any] | str:
        ...
        if not isinstance(payload, (dict, list, str)):
            raise ValueError(f"{path.name} has unexpected content type")
        return payload
