...
def map_record(record: LegacyRecord) -> dict[str, Any]:
    ...
    title = _normalize_space(frontmatter.get("title") or metadata.get("title"))
    target = _normalize_space(frontmatter.get("target") or metadata.get("target"))
    ...
    return {
        ...
        "source_digest": legacy_source_hash,
